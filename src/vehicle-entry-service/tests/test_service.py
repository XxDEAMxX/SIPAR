...
from schemas import EntryEventRequest
from service import DuplicateEventGuard, EntryEventDispatcher
...
def test_dispatcher_forwards_first_event() -> None:
    ...
    dispatcher = EntryEventDispatcher(client, DuplicateEventGuard(window_seconds=10))

    event = EntryEventRequest(
        plate="abc123",
        entry_time=datetime(2026, 4, 8, 17, 30, tzinfo=timezone.utc),
        source="manual",
    )
...
def test_dispatcher_skips_duplicate_within_window() -> None:
    client = FakeBackendClient()
    dispatcher = EntryEventDispatcher(client, DuplicateEventGuard(window_seconds=20))

    first = EntryEventRequest(
        plate="ABC123",
        entry_time=datetime(2026, 4, 8, 17, 30, tzinfo=timezone.utc),
        source="manual",
    )
    second = EntryEventRequest(
        plate="ABC123",
        entry_time=datetime(2026, 4, 8, 17, 30, 10, tzinfo=timezone.utc),
        source="auto-camera",
    )
