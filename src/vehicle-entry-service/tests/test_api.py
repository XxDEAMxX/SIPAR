...
from schemas import EntryEventResponse
from settings import ServiceSettings


class FakeDispatcher:
    def __init__(self, result: EntryEventResponse) -> None:
        self.result = result
        self.calls = 0
...
def _settings() -> ServiceSettings:
    return ServiceSettings(
        service_name="vehicle-entry-service-test",
...
def test_register_vehicle_entry_ok() -> None:
    app = create_app(_settings())
    app.state.dispatcher = FakeDispatcher(
        EntryEventResponse(
            ...
            message="ok",
            plate="ABC123",
            entry_time=datetime(2026, 4, 8, 18, 0, tzinfo=timezone.utc),
        )
    )
    client = TestClient(app)
    ...
    response = client.post(
        ...
        json={
            "plate": "abc-123",
            "entry_time": "2026-04-08T18:00:00Z",
            "source": "manual",
        },
    )
...
def test_register_vehicle_entry_backend_error() -> None:
    app = create_app(_settings())
    app.state.dispatcher = FakeDispatcher(
        EntryEventResponse(
            accepted=False,
...
def test_detect_frame_endpoint_registers_event() -> None:
    ...
    response = client.post(
        "/api/v1/vehicle-entries/detect-frame",
        json={
