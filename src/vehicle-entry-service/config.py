...
def load_settings() -> Settings:
    raw_camera_source = os.getenv("CAMERA_SOURCE", "http://127.0.0.1:8010/cameras/entrada/stream").strip()
    camera_source: int | str = int(raw_camera_source) if raw_camera_source.isdigit() else raw_camera_source
    return Settings(
        camera_source=camera_source,
        camera_id=os.getenv("CAMERA_ID", "entrada"),
        ...
        detection_direction=os.getenv("DETECTION_DIRECTION", "entry").strip().lower(),
        source_name=os.getenv("SOURCE_NAME", "vehicle-entry-service").strip(),
